...
// CHALLENGE 7: Get the sum of property of nested array of objects with infinite depth
...
const total = (person) => {
	return person.reduce((acc, currentValue) => {
		acc += currentValue;

		if (currentValue.kids) {
			acc += total(currentValue.kids);
		}
...
const getTotalAge = (persons) => {
	return persons.reduce((acc, currentValue) => {
		acc += currentValue.age;

		if (currentValue.kids) {
			acc += getTotalAge(currentValue.kids);
		}
...
console.log(getTotalAge([person]));
